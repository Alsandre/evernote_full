...
export const createNote = (req, res) => {
  ...
  pool.query(userQuery, [req.body.user.email], (userErr, userData) => {
    ...
    if (userData.length > 0) {
      const noteQuery =
        "INSERT INTO notes (`noteTitle`, `content`, `uid`) VALUES (?, ?, ?)";
      pool.query(
        noteQuery,
        [req.body.noteTitle, req.body.content, req.body.user.uid],
        (noteErr, noteData) => {
          if (noteErr) {
...
export const updateNote = (req, res) => {
  ...
  pool.query(checkOwnershipQuery, [userId], (userErr, userData) => {
    ...
    const updateNoteQuery =
      "UPDATE notes SET noteTitle = ?, content = ? WHERE noteId = ?";

    pool.query(
      updateNoteQuery,
      [req.body.noteTitle, req.body.content, req.query.noteId],
